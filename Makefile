...
LDFLAGS=-D_POSIX_C_SOURCE=199309L $(shell pkg-config --cflags gtk+-3.0) -Wl,--whole-archive -I/opt/vc/include -L/opt/vc/lib/ -lGLESv2 -lEGL -lbcm_host -lvchiq_arm -lpthread -lrt -ldl -lm -Wl,--no-whole-archive -rdynamic $(shell pkg-config --libs gtk+-3.0) $(shell pkg-config --libs libavcodec libavformat libavutil libswscale) -lasound $(shell pkg-config --libs gtk+-3.0) $(shell pkg-config --libs sqlite3)
SOURCES=VSHost.c VStudio.c VSJam.c VSEffect.c VSTMediaPlayer.c VSTVideoPlayer.c AudioDev.c AudioMic.c AudioSpk.c AudioMixer.c AudioPipeNB.c AudioEncoder.c VideoQueue.c YUV420RGBgl.c
OBJECTS=$(SOURCES:.c=.o)
EXECUTABLE=VSHost
